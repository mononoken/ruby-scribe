...
RSpec.describe NotePolicy, type: :policy do
  ...
  describe_rule :create? do
    let(:context) { {user:, journal: record.journal} }

    succeed "when user is allowed to manage the note's journal" do
      before do
    ...
    failed "when user is not allowed to manage the note's journal" do
      before do
        allow(policy).to receive(:allowed_to?)
          .with(:manage?, record.journal)
          .and_return(false)
